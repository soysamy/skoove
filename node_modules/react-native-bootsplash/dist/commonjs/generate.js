...
const xcassetName = "BootSplashLogo"; // https://github.com/androidx/androidx/blob/androidx-main/core/core-splashscreen/src/main/res/values/dimens.xml#L22
...
const log = function (text) {
  let dim = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  console.log(dim ? _picocolors.default.dim(text) : text);
};
...
const generate = async _ref2 => {
  ...
  const logWrite = (emoji, filePath, dimensions) => log(`${emoji}  ${_path.default.relative(workingPath, filePath)}` + (dimensions != null ? ` (${dimensions.width}x${dimensions.height})` : ""));
  ...
  if (assetsPath && _fsExtra.default.existsSync(assetsPath)) {
    ...
    await Promise.all([{
      ...
      return image.clone().resize(width, height).quality(100).writeAsync(filePath).then(() => {
        logWrite("✨", filePath, {
          width,
          height
        });
  ...
  if (android) {
    ...
    await Promise.all([{
      ...
      const width = logoWidth * ratio;
      const height = getHeight(width);
      const canvasSize = splashScreenIconSizeNoBackground * ratio; // https://github.com/oliver-moran/jimp/tree/master/packages/jimp#creating-new-images

      const canvas = new _jimp.default(canvasSize, canvasSize, 0xffffff00);
      const logo = image.clone().resize(width, height);
      const x = (canvasSize - width) / 2;
      const y = (canvasSize - height) / 2;
      return canvas.blit(logo, x, y).quality(100).writeAsync(filePath).then(() => {
        logWrite("✨", filePath, {
          width: canvasSize,
          height: canvasSize
        });
 ...
 ${_picocolors.default.blue("┃")}  One-time amounts are available.              ${_picocolors.default.blue("┃")}
 ${_picocolors.default.blue("┃")}  ${_picocolors.default.underline("https://github.com/sponsors/zoontek")}          ${_picocolors.default.blue("┃")}
 ${_picocolors.default.blue("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛")}
`);
  log(`✅  Done! Thanks for using ${_picocolors.default.underline("react-native-bootsplash")}.`);
};
